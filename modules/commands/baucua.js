...
module.exports.config = {
  ...
  module.exports.run = async function({ api, event, args, Currencies, getText, permssion }) {
    try {
      ...
      if (isNaN(moneyBet) || moneyBet <= 50) return api.sendMessage('Số tiền đặt cược không được dưới 50VND', threadID, messageID);
      ...
      if (content == 'gà' || content == '🐓') {
        ...
        return api.sendMessage(`Sai định dạng\n${global.config.PREFIX}${this.config.name} <[gà/tôm/bầu/cua/cá/nai] hoặc[🐓/🦞/🍐/🦀/🐟/🦌]> <Số tiền cược (lưu ý phải trên 50VND)>`, threadID, messageID);
      }
      // request
      ...
      // sendMessage
      api.sendMessage({
        body: '🎄 Nghiện ngập không tốt đâu nhé !!\n😵 Đợi tao lắc đã 😵',
